...
///
/// Blue fill — dark-mode contract: content uses `Color.risoOnColor` (the
/// static on-colour cream, web `--riso-on-color`), matching `RisoButton`'s
/// `.blue` kind (`RisoControls.swift`: fill `.risoBlue` → `.risoOnColor`).
...
/// which is nearly illegible. The cream is static (it reads against the
/// fill, not the page), so it does not flip with the theme.
struct RisoCounterLinkHintView: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 8) {
            ...
            // on-colour cream ring + text, matching web's `.hintPill` treatment
            ...
            Button(action: onToggle) {
                Text(linked ? "Don't link" : "Link")
                    ...
                    .overlay(Capsule().strokeBorder(Color.risoOnColor, lineWidth: Riso.Keyline.container))
            }
            .buttonStyle(.plain)
        }
        .padding(10)
