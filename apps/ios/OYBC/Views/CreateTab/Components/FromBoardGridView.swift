...
struct FromBoardGridView: View {
    ...
    private func handleDeriveSave(source: OYBC.Task) {
        // Shared derive action (extract-at-three) — keeps the sheet open
        // until the write succeeds or fails visibly, as before.
        DeriveCounterAction.createDerived(
            source: source,
            goalInput: deriveMaxCountInput,
            userId: userId,
            onError: { deriveError = $0 },
