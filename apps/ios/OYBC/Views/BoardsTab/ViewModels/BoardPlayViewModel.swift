...
final class BoardPlayViewModel: ObservableObject {
    ...
    /// `flashEvent` one-shot pattern (monotonic id + `Equatable`).
    @Published var editEvent: BoardPlayEditEvent? // internal for the +EditCommit extension split (2026-09-15)

    /// Monotonic counter backing `editEvent.id` so consecutive emissions are
    /// distinct `Equatable` values for the view's `.onChange`.
    var editEventCounter = 0 // internal for the +EditCommit extension split (2026-09-15)
    ...
    private(set) var userId: String? // internal for the +EditCommit extension split (2026-09-15)
    let database: AppDatabase // internal for the +EditCommit extension split (2026-09-15)
