    ...
    static func spawnTemplateBoard(_ spawn: PendingTemplateSpawn) throws -> RecurringSpawnOutcome {
        ...
        for (cell, t) in placement.enumerated() {
            ...
            boardTasks.append(BoardTask(
                ...
                // Set explicitly (not defaulted to nil) so the iOS-pushed
                // Firestore payload matches the web spawn's `false` and
                // a future achievement-square cross-board feature can
                // trust the field is always populated on spawned cells.
