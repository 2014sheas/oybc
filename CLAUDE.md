...
- **Wire up streaks (important; pre-launch).** There is currently **no streak model anywhere** — yet a hardcoded `"7d STREAK"` is shown as if real in the GREENLOG celebration overlay (`RisoGreenlogOverlay.swift:114`) and baked into shared social posters (stats-on by default, `ShareBoardSheet.swift:33,191`), and onboarding copy promises streaks + a "GREENLOG history" (`OnboardingView.swift:230,237,468`). Build a real streak (daily-activity) model + compute the value end-to-end. **Until it ships, that "7d" is fabricated data visible in-app and in posts users share** — if streaks won't make the launch cut, remove those two stat cards and fix the onboarding copy as a fallback before shipping.
