...
- **Web unit-test harness — SHIPPED** (Track E2, riding in on B2): Vitest + fake-indexeddb wired into `apps/web` (`apps/web/vitest.config.ts`); `pnpm -w test` runs it in CI via turbo. As of this writing: 8 test files / 49 tests, covering `db/operations` (shared-counter completion cascade, deletion, greenlog/bingo-gating orchestration). `packages/shared` still covers the cross-platform pure-logic surface separately. Broadening coverage (wizard persist, `useBoardWizard` reducer, more of `db/operations`) is Track E3 in `docs/ROADMAP.md`.
