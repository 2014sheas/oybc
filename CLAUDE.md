...
- Create-tab containers were thinned to routing-only shells: web `CreateHubPage.tsx` is 207 LOC with the two URL-param effects + draft-resume callback extracted to dedicated hooks under `apps/web/src/pages/createHub/` (`useRecurringTimeframeParam`, `useEditTemplateParam`, `useResumableDraft`). iOS `CreateHubView.swift` is 221 LOC with hub mode + drafts + library count + the four GRDB loaders moved to `CreateHubViewModel` (mirrors `BoardWizardViewModel`'s `@Observable` pattern). Form-state hooks/view-models (`useCreateFormState.ts`, `CreateFormViewModel.swift`) were already extracted in Phase 6.3.
