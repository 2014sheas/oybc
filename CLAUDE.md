...
- **Web Account & security** (the §5c web counterpart) — iOS shipped the screen + the shared `onUserDeleted`/`deleteUserData` Cloud Functions; web has no account-security page yet. When web ships it, reuse the `deleteUserData` callable (the Firestore purge is platform-agnostic) and mirror the change-email/password + provider-link flows with the Firebase JS SDK. (The earlier deploy prereq is **resolved**: `oybc-dev-e2668` is on Blaze and both functions are deployed — see Cloud Functions above — so the Firestore purge runs server-side; no longer a blocker.) Not yet a `docs/ROADMAP.md` track — pick up by directive.
- `autoArchiveCompleted` remains the one still-unwired housekeeping pref (no archive logic consumes it on either platform). Tracked as `docs/ROADMAP.md` Track E5.
- **Web draft-board containment** (the iOS-only counterpart of `bugfix/draft-board-task-leakage`; tracked as `docs/ROADMAP.md` Track C6) — iOS shipped two draft fixes that web should mirror when it gets there: (1) **Draft tasks hidden until active** — wizard-born tasks (deferred-persist, Bug #85) are marked `Task.createdInWizard` and hidden from library-BROWSE surfaces until placed on a non-draft board (visibility rule: hide iff `createdInWizard` AND every live placement is a draft board; pure-derived, no clearing). `createdInWizard` is in the shared `Task` type + `TaskSchema` and round-trips via sync, but **only iOS sets it** (`CreateFormViewModel` deferred branches) **and filters on it** (`TaskLibraryViewModel.browsableTasks`, consumed by the Tasks tab + wizard "add from library"). Web's `useTaskLibrary` / Tasks page still browse the full set and web's wizard doesn't set the flag (confirmed still true as of this writing — no `createdInWizard` reference anywhere under `apps/web/src`). (2) **Drafts never playable** — iOS routes draft taps (Boards list cards, core-grid slots, core-window pager) to the wizard-resume flow and guards `BoardPlayView` so a draft renders a "Resume draft" prompt instead of the grid; web `BoardPlayPage` / core surfaces still open drafts as playable boards.
