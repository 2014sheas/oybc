...
- **Web draft-board containment** (tracked as `docs/ROADMAP.md` Track C6) — part (1) **SHIPPED on web** via PR #203 (2026-07-02): wizard-born draft tasks are marked `Task.createdInWizard` on both platforms and hidden from library-BROWSE surfaces via the shared `computeBrowsableTasks` (`packages/shared`) — web sets the flag (`useCreateFormState.ts` deferred branches) and filters through `useBrowsableTasks`; iOS via `TaskLibraryViewModel.browsableTasks`. Part (2) **still open on web**: iOS routes draft taps (Boards list cards, core-grid slots, core-window pager) to the wizard-resume flow and guards `BoardPlayView` so a draft renders a "Resume draft" prompt instead of the grid; web `BoardPlayPage` / core surfaces still open drafts as playable boards (the A1b follow-up).
