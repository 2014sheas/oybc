...
- `docs/BOARD_INTEGRITY.md` — the 2026-07-24 four-auditor board/placement-pipeline audit + the five-PR hardening program (issues #358–#362): the tombstone defect (PR-1 #358, shipped — `BoardTask` gained `isDeleted`/`deletedAt` like every other synced collection), placement-integrity repair + determinism + sealed guards (PR-2 #359), and the unified per-cell board resolver (PR-3 #360, closes the web-only achievement-render bug). Read before touching `BoardTask` deletion, the boardTasks-pull cascade, or any per-cell "is this complete?" render logic.
