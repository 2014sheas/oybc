...
- **Pre-launch polish — ungated `print()` (resolved pending the 2026-09-23 audit cleanup PR):** the earlier sweep DEBUG-gated / `dlog`-routed Release logging, but as of 2026-09-23 exactly two `print(` calls remained outside `#if DEBUG`/`dlog` (`AppDatabase+Boards.swift` `saveWizardBoard` and `AppDatabase+RecurringTemplates.swift` `spawnRecurringBoard` "resolved twice" warnings); the audit cleanup PR converts them. (Tracked as `docs/ROADMAP.md` Track E5.) *(The Edit-profile "Blip mood" picker — a dead control that persisted nothing — was removed in the obsolete-controls sweep; the Blip mascot itself was later retired entirely in favour of the `RisoMiniBoardArt` mini-board motif + `RisoInitialAvatar` initials avatar — see the Blip-retirement handoff PR.)*
