...
properties — (1) *is it a free space?* (`FREE`, auto-completed, no
...
free space or task square. That maps to `FREE` ⇄ `NONE`. (The general
...
(positional center) && centerSquareType ≠ NONE`** (i.e. `FREE`/`CHOSEN`
...
- **Free space → task square:** `FREE` → `NONE`. The cell becomes a
