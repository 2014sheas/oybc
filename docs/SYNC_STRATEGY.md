...
>
> **Note (2026-07-08):** Section #1 (**ProgressCounter Conflict Resolution**) is **superseded** — `ProgressCounter` was never revived; Shared Counters (Issue #84) shipped the per-Task `sharedCounterId`/`baseline`/`lastSyncedCount` model instead, with its own additive-merge design. Section #1 is kept below as historical reference for how the LWW-vs-additive tradeoff was originally reasoned about (the "Strategy A vs Strategy B" framing), but the **shipped, current-state design lives in [§Shared Counter Sync — current state (shipped)](#shared-counter-sync--current-state-shipped)**, added below. That new section also folds in the D1/D2/D3 sync-hardening work (dead-letter surfacing, reliable ancestor advancement, per-entity queue coalescing).
...
If only one side diverged from `lastSyncedCount`, that side's value is simply authoritative (no real conflict — the "conflict" is just one device catching up). The merge function is `sharedCounterMerge` (`packages/shared/src/algorithms/sharedCounterMerge.ts`, Swift twin `SharedCounterMerge.swift`), fixture-tested against both platforms (Track C1, 21 vectors). It runs on both the safety-net pull path and the real-time listener pull path — same merge logic, two entry points.

### D2 — reliable `lastSyncedCount` advancement (issue #294, shipped)

The merge's correctness depends on `lastSyncedCount` actually advancing after every push — if the post-push bookkeeping write silently fails, the next conflict silently degrades from additive-merge to plain LWW (the exact increment-losing behavior the merge exists to prevent). D2 closed that gap: the queue-item completion and the `lastSyncedCount` advance now land in **one transaction** (web: `completePushedItemLoud` in `apps/web/src/firebase/syncService.ts`; iOS: the mirrored `SyncService.swift` path). A failure re-throws so the push loop marks the item FAILED and retries — the advance is idempotent (it sets the ancestor to the value just pushed), and a loud `console.error` / log names the semantic consequence instead of swallowing it.
...
| --- | --- | --- |
| Lost increments across concurrent offline edits | Additive three-way merge via `lastSyncedCount` | `sharedCounterMerge.ts` / `SharedCounterMerge.swift` |
| Ancestor-advance reliability | D2 — atomic completion + advance, retry-on-failure | `syncService.ts` `completePushedItemLoud` / iOS mirror |
