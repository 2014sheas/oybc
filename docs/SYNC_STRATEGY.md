...
**Phase 2 (preset-pool boards)** will add a new collection `users/{uid}/recurring_board_templates`. Versioning, LWW resolution, and soft-delete tombstone semantics mirror `boards` exactly. The collection name is added to the sync service's known-collections list; no new conflict-resolution logic is required.

**Phase 3 (board-completion-as-a-square)** extends achievement squares with one new optional field — `referencedBoardId` — on the existing `board_tasks` collection. No new collection, no schema migration to a different table. Cross-board cascade fires inside the existing `runBoardCascadeForTask()` pipeline by adding a parallel fan-out for `board_tasks WHERE referencedBoardId = this.id`; no new sync paths are added.
