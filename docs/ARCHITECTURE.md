...
## Wizard "From a board" picker — RETIRED

> Designed 2026-06-01, shipped via PR #82 (Riso-reskinned in PR #128), **retired in Plan A** of the Board Sources member-rules train (design locked 2026-09-17). Canonical record of the replacement: [`docs/BOARD_SOURCES.md` §Member rules](BOARD_SOURCES.md#member-rules--counting--compound-tasks-pulled-from-sources-design-locked-2026-09-17).

The picker let the wizard's Library sheet swap to a mini-grid list of eligible boards and then to one board's actual grid, with per-square tap = link and long-press = `⎘ Add a copy of this task…`. It was retired because (a) the "Add from a pool or board" Sources sheet already pulls ranges of squares from a board, (b) per-member control for counting/compound squares is better expressed as member rules on those source rows than as a per-square copy modal, and (c) the surface was the recurring cause of wrong-surface fixes (CLAUDE.md §Board-creation surfaces). Deleted on both platforms: the picker + grid views, the Copy modal/sheet, `useSourceBoards`/`useSourceBoardPlacements` ↔ `SourceBoardsViewModel`, `fetchEligibleSourceBoards`, and `copyTask`/`copyCompound`. The shared `isEligibleSourceBoard` rule survives — it feeds `fetchSourceSheetBoardEntries`. Design history lives in the git log of this section (`git log -p --follow docs/ARCHITECTURE.md -S 'From a board'`).
...
- **Windowed evaluation** — a task's state on a board is computed against that board's window `[startDate, endDate]` (events with `startDate <= occurredAt <= endDate`, parsed-instant compare; open-ended when `endDate` is null; amended 2026-09-24 from the original `[startDate, ∞)`), not a global bit. A log made from an ended-but-unsealed board's own surface is stamped at its `endDate` so it counts there and not for the next window. The same task legitimately shows different states on today's board vs this month's. `Task.isCompleted`/`currentCount`/`completedAt` are demoted to **lifetime caches** — read by library/global surfaces, recomputed from events on pull, never trusted for windowed board grids.
