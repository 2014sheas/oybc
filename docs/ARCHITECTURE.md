...
The create-task interface lets a user pick an existing counting task as a "template" (`CountingTemplatePicker` / `CountingTemplatePickerView`), but the picker only prefills `action` + `unit` into a brand-new, fully independent task — there is no mechanism for two distinct counting tasks to share a running total. `ProgressCounter` + `TaskProgressCounter` + `calculateCountingRollup()` exist on both platforms as dead scaffolding (zero callers, not in `SYNCABLE_COLLECTIONS`). This section locks the seven design decisions called out in [Issue #84](https://github.com/2014sheas/oybc/issues/84) so the Phase 1 playground spike, Phase 2 UI, Phase 3 increment integration, and Phase 4 sync can proceed without re-litigating the model on each PR.
...
- **No cycles in `sharedCounterId`** — a Task `A` with `sharedCounterId = B` cannot itself be a source. Enforce via Zod refinement on `Task` creation/update: reject if any other Task already has `sharedCounterId = thisTask.id`. (Two-level chains would force the derivation pass to recurse beyond depth 1 and make the additive-merge semantics ambiguous — what's the "source of truth" accumulator when there are three of them? Not a feature, just a footgun.) Same pattern as Phase 6.3's `hasCycle` for ACHIEVEMENT tasks — the algorithm lives in `packages/shared/src/algorithms/` and runs at write time.
...
- **Phase 1 — playground spike**: shared-counter math in a `SharedCounterPlayground.tsx` + `SharedCounterPlaygroundView.swift` pair. Increment a source, derive `displayed` + `isCompleted` for N linked tasks, render the results in real reusable components per the [[feedback_playground_real_components]] rule. No boards, no real DB writes — synthetic in-memory fixtures only. Verifies the math + the derivation helper + the no-overshoot-clamp invariant.
