...
| **B1** | Shared types (`memberRules`, `manualTaskVary`) + Zod + Swift mirrors; draft-blob additive (still v2); GRDB **v31** column; pure helpers (`nominalWindowDays`, `autoTarget`, `varyRange`, `rollTarget`, `applyMemberRules`, `planDerivedTasks`) + mirrored vectors. Inert — nothing writes rules yet. **SHIPPED** (#<n>). | lockstep |
...
Remaining B2 items surfaced during B1 review: (a) **R9** — iOS has no `fetchCompoundChildrenByCompoundIds` (Plan A's KEEP note is web-only; iOS has `fetchCompoundChildren(compoundTaskId:)`) — B2 adds a batched iOS fetch. (b) two selected *members* sharing a root could yield a duplicate placement id (unreachable via `selectBoardTasks` family exclusivity) — B2's persist dedupes `placementIds` as a guard; the intra-compound twin of this is fixed in B1 (R13). (c) Swift `sort` is unstable — B1 made both comparators total (R14); B2 still adds a write guard for unique `childIndex` per compound. (d) `dayNumber` edge semantics diverge from JS `Date.UTC` for year 0–99 / out-of-range parts — unreachable from real ISO dates. (e) **Pre-existing, NOT introduced by B1:** iOS encodes the five JSON-TEXT fields of a `RecurringBoardTemplate` — `sources`, `poolIds`, `manualTaskIds`, `removedTaskIds`, and now `manualTaskVary` — as JSON *strings* on the sync wire, while web's `pullApply` validates pulled `recurringBoardTemplates` with `RecurringBoardTemplateSchema` (arrays/records, no `preprocess`) and on a miss skips the WHOLE row — so an iOS-authored repeating board's record may not land on web at all. Tracked as its own bugfix (GitHub issue #<issue>), not a B2 rider; must be verified on a real two-platform account before B2 makes member rules depend on these records crossing platforms. (f) the rules-cap emulator test above.
