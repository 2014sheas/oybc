...
Remaining B2 items surfaced during B1 review: (a) iOS has no `fetchCompoundChildrenByCompoundIds` (Plan A's KEEP note is web-only; iOS has `fetchCompoundChildren(compoundTaskId:)`) — B2 adds a batched iOS fetch. (b) two selected members sharing a root could yield a duplicate placement id (unreachable via `selectBoardTasks` family exclusivity) — B2's persist dedupes `placementIds` as a guard. (c) Swift `sort` is unstable — B2 adds a write guard for unique `childIndex` per compound. (d) `dayNumber` edge semantics diverge from JS `Date.UTC` for year 0–99 / out-of-range parts — unreachable from real ISO dates. (e) iOS puts `manualTaskVary` on the sync wire as a JSON string exactly like the other four such fields — any peer-side shim is a B2/B3 item. (f) the rules-cap emulator test above.
