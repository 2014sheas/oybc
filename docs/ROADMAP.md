...
- **Ended-but-unsealed boards — DECIDED 2026-09-24 (option (c), both), PR #507 — `fix/root-window-end-bound`:** root squares used to keep absorbing later logs on an ended-but-unsealed board (start-bound-only windows). The owner chose both fixes. (1) WC Decision 1 amended: every board's root squares evaluate events inside its own `[startDate, endDate]` (inclusive, parsed instant; sealing narrows to `min(endDate, sealedAt)`), and a log made from an ended board's own surface is stamped `min(now, endDate)` (`lateLogOccurredAt` / `lateLogStampForBoard`) so it counts there and not for the next window — see `docs/WINDOWED_COMPLETION.md` §Overtime attribution. (2) Ended boards are never Board Sources: `isEligibleSourceBoard(board, now)` admits only open, unsealed boards (the #482 30-day lookback is gone), a series binds to its instance open now (`pickOpenSeriesInstance`), and a source with no open board shows "No board for this window yet" — see `docs/BOARD_SOURCES.md` §Boards as sources.
