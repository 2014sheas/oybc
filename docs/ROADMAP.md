...
- **Owner decision pending — ended-but-unsealed boards:** by Windowed Completion Decision 1, root squares on an ended-but-unsealed board keep absorbing later logs. Options: (a) end-bound every board window and stamp logs made from an ended board's own surface at its `endDate`; (b) stop offering ended/sealed boards as wizard sources; (c) both. Not a task until decided.
