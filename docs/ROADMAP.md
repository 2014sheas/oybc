...
- **Acceptance:** nothing deploys to the user-facing project without an explicit release action.
  Prod `functions/.env.<prod-project-id>` sets `REVENUECAT_ALLOWED_ENVIRONMENTS=PRODUCTION` (the code default already fails closed; the committed file keeps non-interactive deploys from failing and makes the setting explicit — see `functions/README.md`).
...
### B6 — Second-wave god-file splits + pull-apply dedup (2026-08 audit) — `L`
- **Why:** the 2026-08 deep-dive audit found the architecture *invariants* clean but flagged files that grew back past a holdable size after (or around) B1/B2 — their bulk is exactly what let the earlier board-integrity defects hide. Sizes at audit time: `BoardPlayView.swift` (~2.4k), `SyncService.swift` (~2.3k), `BoardPlayViewModel.swift` (~2.1k), `AppDatabase+Tasks.swift` (~1.2k), `apps/web/.../BoardPlaySurface.tsx` (~1.4k), `useBoardWizard.ts` (~1.1k), `BoardWizardTasksStep.tsx` (~1.0k). Separately, `SyncService.swift`'s two pull paths (`processPullCollection` ≈ `applyRemoteSubdoc`) carry a near-identical upsert + resolveConflict + reassert-local-win + cascade + sealed-re-derive body (audit architecture M2) — the highest-value single extraction because it's the correctness-critical path and duplication there is where a fix can land on one path and miss the other.
- **Scope:** deliberate, one file at a time, behavior-preserving, tests/snapshots as the safety net (NOT a rushed sweep). Start with the `SyncService` pull-apply dedup (unify the two bodies behind one helper; parallel change to both pull paths — mind the Swift `if let x = throwingFn()` no-implicit-`try` trap, which requires a two-step bind on each path). Then split the render-heavy views along existing seams (per-section subviews / step components) the way B1/B2 did. Skip files that are large but cohesive if a split would only scatter them.
- **Acceptance:** the pull-apply body exists once; no *new* god-file regressions; each split PR green on both platforms with unchanged snapshots/logic tests. Not launch-blocking — pick up by directive.

### B7 — iOS pull off the MainActor + a batched tasks pull branch (2026-09 audit) — `L`
- **Why:** the 2026-09-23 audit ([report](https://claude.ai/artifact/QdXwwtqiXrgQi5YauXHgrs)) found the iOS pull doing its GRDB work on the main actor: `SyncService` is `@MainActor`, `processPullCollection` opens one `AppDatabase.shared.write` **per pulled document**, and each one runs `runPullCascade`, which re-fetches the whole workspace (`CompoundChild`/`BoardTask`/`Task`/`Board` `.fetchAll`) per changed task. A large pull (fresh install, long offline stretch) is O(docs × workspace) on the UI thread.
- **Scope:** give the `tasks` collection a batched branch the way `taskEvents` already has one (`applyTaskEventsBatch`: apply every row, then recompute once — `runPullCascadeForTasks(db:changedTaskIds:)` is the existing set-based cascade to call); move the database work off `@MainActor` so only `@Published` state is mutated there. Fold this into B6's `processPullCollection` ≈ `applyRemoteSubdoc` dedup rather than doing it twice — one pull-apply body, batched.
- **Acceptance:** a pull of N task docs runs one write transaction and one cascade; no GRDB write executes on the main actor during pull (assert in a `SyncService` pull-apply test); the rollback-on-cascade-failure seam test (E3) still passes.

### B8 — Pre-emptive god-file splits (2026-09 audit) — `M`
- **Why:** the file-size guardrail only fires once a file crosses the 1000-line ceiling, and several files are sitting right under it. `AppDatabase+DerivedCounters.swift` is at the ceiling (one more line fails CI). Two single functions are god-functions on their own: `BoardWizardViewModel.init` (~280 lines of per-hydration-source setup) and `planDerivedTasks` (~300 lines in `BoardSourceMemberRules.swift`, TS twin in `packages/shared/src/algorithms/memberRules.ts`, with the no-identical-clone guard written out twice).
- **Scope:** split `AppDatabase+DerivedCounters.swift` along its existing `// MARK:` seams (Mint / Refresh (non-authored) / Board-deletion sweep + Retire); break `BoardWizardViewModel.init` into one helper per hydration source; extract a `derivedOrRoot()` helper in `planDerivedTasks` on **both** platforms so the clone guard exists once (`memberRuleVectors.json` pins behaviour — vectors must pass unchanged). Then the other files within ~50 lines of the ceiling — list them at pickup (`git ls-files apps packages functions | grep -E '\.(swift|tsx?)$' | xargs wc -l | awk '$1>=950 && $1<=1000'`) and split only where a seam exists.
- **Acceptance:** no source file at or within 50 lines of the ceiling without a written reason; `planDerivedTasks` contains one clone guard per platform; vectors + snapshots unchanged.

### B9 — iOS Views → ViewModels (finish the B3 seam) — `L`
- **Why:** B3 gave ViewModels an injected `AppDatabase`, but many Views still reach `AppDatabase.shared` directly, which keeps their logic untestable against `makeTestInstance()`. PR #498 (`refactor/ios-db-seams`) moves the task-detail/tasks-tab/persist call sites; what it leaves is this roster: `BoardListView`, `CounterDetailView`, `CountersHubView`, `BoardSettingsView`, `PoolEditSheetView`, `RisoSpecialTaskPanel`, `ProfileView` (plus the single-use sites the grep below shows), and `NewTaskSheetView` / `PoolEditSheetView` / `CounterDetailView` take no `database:` parameter at all. `CreateHubViewModel.reloadDrafts` inlines a GRDB `Board.filter(... status == "draft" ...)` query that belongs in the data layer.
- **Scope:** per view, move the DB calls into a ViewModel (or the view's existing one) that takes `database: AppDatabase = .shared`; add `AppDatabase.fetchDraftBoards(userId:)` and call it from `reloadDrafts`. From PR #498's review: move `EditTaskSheet.Patch` next to its DB helper as `TaskEditSheetPatch` (`TaskEditPatch` is already taken by the wizard's staged-edit type), and route the two "not done" subtitle counts through `BoardSources.availableSupplyIds(...).count`. Measure progress with `grep -rc 'AppDatabase.shared' apps/ios/OYBC/Views` rather than a number written here.
- **Acceptance:** `AppDatabase.shared` appears in `Views/` only in an `= .shared` default argument; each moved ViewModel has at least one `makeTestInstance()` test.
...
### C7 — Sign-out local-data wipe (owner decision) — `S`
- **Why:** both platforms' sign-out clears only the sync queue (web `signOut` in `firebase/authService.ts`, iOS `AuthService.signOut()`); the signed-out user's rows stay in IndexedDB / SQLite. The UI filters by `userId`, so the next account never *sees* them, but on a shared device they are readable through devtools or a jailbroken filesystem. Wiping costs a full re-pull on the next sign-in of the same account.
...
### C8 — On-colour contract (content on red/blue/green fills) — `S` — DECIDED: adaptive on iOS, static on web
- **Why:** web puts static cream `--riso-on-color` on red/blue/green fills (`docs/RISO_WEB.md` token table); iOS uses the adaptive `Color.risoPaper` in the same role (`RisoControls.swift` button/chip foregrounds, `CreateHubBoardCTAView.swift`, `RisoBingoToast.swift`). Light-mode contrast improves either way (cream is lighter than paper on the dark light-mode fills); in DARK mode the lightened fills (`#3BCB92` / `#FF6A4A` / `#6678F2`) put cream at roughly 1.9:1 / 2.6:1 / 3.5:1 versus ~9:1 / 6.5:1 / 4.9:1 for the old flipped-ink paper — below WCAG AA on green and red. Web has shipped that way since the Riso pass; the follow-up is to deepen the dark red/green fills on BOTH platforms so cream reaches AA, not to re-split the platforms.
- **Scope:** pick one and write the rule into both `docs/RISO_UI_CHECKLIST.md` and `docs/RISO_WEB.md` — see the Decision bullet below.
- **Acceptance:** both checklists state the same on-colour rule; this entry records which one was chosen and the PR.
- **Decision (2026-09-24, `fix/audit-t2-minors-b`, owner):** the platforms **diverge on purpose**. Web keeps static `--riso-on-color` (`#FBF6EA`); iOS keeps adaptive `Color.risoPaper` on red/blue/green/achievement fills. A static-cream conversion of ~60 iOS sites was built and measured (real renders side by side: cream on the lightened dark fills ≈1.9:1 green / 2.6:1 red / 3.5:1 blue vs ~8.9 / 6.5 / 4.9:1 for adaptive ink) and the owner chose readability over pixel parity; the conversion was reverted before merge. Both checklists state the divergence. Re-open only with deeper dark fills on both platforms (the lightest Riso-hue fills that reach 4.5:1 with cream: `#227E5A` / `#D92600` / `#4A5FF0`).
...
- ~~CAPTCHA / auth rate-limit hardening~~ — **moved to E9** (pre-launch abuse controls: App Check, CI secret split, unsubscribe tokens).

### E8 — Vector files for the hand-mirrored twins that have none — `M`
- **Why:** C1's fixture pattern covers the algorithms it started with, but these TS ↔ Swift pairs are still mirrored by hand with no shared vectors: recurring detection (`findPendingRecurringBoards` / `getCoreBoardSlots` / `isFreshlyDealtBoard` — `recurringBoards.ts` ↔ `RecurringBoards.swift`) and spawn (`findTemplatesPendingSpawn` / `validateSpawnPool` / `buildSpawnPlacement` — `recurringBoardTemplates.ts` ↔ `RecurringBoardTemplates.swift`); `formatWindowLabel` (`calendarBoundaries.ts` ↔ `TimeframeFormatting.swift`) and `boardDisplayName` (↔ `BoardDisplayName.swift`); the DB-level source resolution (`resolveSourceBoard` / `resolveBoardSourceSupply` in web `db/operations/boardSources.ts` ↔ `AppDatabase+BoardSources.swift`'s `resolveSourceBoard` / `resolveSupply` — a fix for the series tie-break drift, with a vector, is planned in the 2026-09 audit's T2 (`fix/audit-t2-minors`), not shipped yet); `computeBrowsableTasks` (`browsableTasks.ts` ↔ `BrowsableTasks.swift`). The iOS test target also re-declares private `make*` builders file by file instead of sharing them, and two test files are past 1000 lines (`BoardPlayViewModelTests.swift`, `MemberRuleVectorTests.swift`).
- **Scope:** one `packages/shared/tests/fixtures/*Vectors.json` per pair above, loaded by Jest and by a `*VectorTests.swift` twin (the C1 recipe, e.g. `BoardSourceVectorTests.swift`). Add `OYBCTests/TestFixtures.swift` and migrate the private `make*` builders to it incrementally (count them at pickup with `grep -rhoE 'private (static )?func make[A-Z]\w*' apps/ios/OYBCTests`). Split the two oversized test files along their `// MARK:` groups.
- **Acceptance:** changing an expected value in each new fixture fails both suites; no test file over 1000 lines.

### E9 — Pre-launch abuse controls — `M`
- **Why:** anonymous auth (guest mode) means anyone can mint a uid and call the callables; today no Cloud Function sets `enforceAppCheck` and neither client initialises App Check. CI deploys with one `FIREBASE_SERVICE_ACCOUNT` secret shared by the PR-preview Hosting job (`hosting.yml`) and the push-to-dev deploy jobs (`hosting.yml`, `functions.yml`, `firestore-rules.yml`), so a PR-triggered job holds deploy-grade credentials. The coming-soon unsubscribe token is the signup doc id — `sha256(lowercased email)` (`unsubscribe` in `functions/src/index.ts`) — so anyone who knows an address can unsubscribe it; `subscribe` stores the client-supplied `source` string unbounded.
- **Scope:** App Check (DeviceCheck/App Attest on iOS, reCAPTCHA Enterprise on web, `enforceAppCheck: true` on the callables); split the secret into a Hosting-only SA for the PR preview job and a deploy SA for push-to-dev jobs, with GitHub Environment protection on the deploy jobs; HMAC (keyed) or per-row random unsubscribe tokens; bound `source` (allow-list or length cap). Absorbs E5's CAPTCHA / auth rate-limit bullet.
- **Acceptance:** callables reject requests without a valid App Check token; the PR preview job cannot deploy functions or rules; an unsubscribe link cannot be derived from an email address; `source` is bounded.

### E10 — Audit follow-ups (2026-09) — `S` each
Smaller items surfaced while reviewing the 2026-09 audit fix train and its sibling PRs.
- **iOS sync uid guard on pull:** `pushSyncCore` skips on a uid mismatch, but `fullSync`/`pullSync` do not (web guards push and pull via `assertSyncUserMatches`). Guard pull too, and extract a pure `pushAllowed(currentUid:userId:)` so the pass path is testable — today only the skip path has a test.
- **Web counter failure feedback — page wiring tests:** the "Counter not updated" feedback on `CounterLedgerCard` / `CountersHubPage` / `CounterDetailPage` has no page-level test because Vitest runs `environment: 'node'` (`apps/web/vitest.config.ts`); add them once a DOM harness exists.
- **RevenueCat webhook:** `REFUND_REVERSED` is neither a grant nor a revoke in `functions/src/revenueCatWebhook.ts` (fails safe — no change); re-granting on it is a follow-up. `isLifetimeProductId`'s `/lifetime/` match becomes an exact product-id list when monetization #452 lands.
- **iOS `formatWindowLabel` locale:** it formats with `Locale.current` (`TimeframeFormatting.swift`), and healed-name assertions such as `"Mar 15, 2026"` in `SourceSheetBoardEntriesTests` / `CoreBoardWindowViewModelTests` assume en_US — pin the locale in those tests (or in the formatter, if the product wants a fixed format).
- **Sources model (PR #497):** web `BoardSettingsPage` reads the collapsing `useRecurringBoardTemplates` (`[]` while loading), so repeating-board rows first render `seedTaskIds` counts and then change (late-mutation class) — switch to `useRecurringBoardTemplatesQuery`; the pools-mode sequence guard is untestable until the pools state moves into a small loader `ObservableObject` (B3 pattern); `fetchTemplatesReferencingTask` propagates a board-source read error where the roster swallows it — pick one; add draft-capacity fixtures for split-compound / counter-family / chosen-center and a pool-health test with an achievement member.
- **Derived counters (branch `fix/derived-counter-window-freeze`):** the sealed re-derive expansion could be scoped to rows whose window contains a pulled event; `BoardPlayViewModel` resolves each cell's window twice per body pass — cache it; no snapshot covers a window-stamped play cell (blocked on `BoardPlayView` DB injection, see B9); Task Detail's member row shows `currentCount − baseline` (documented behaviour, not a bug).
- **Task-level window columns:** replace per-task `endDate` expiry with board-derived expiry; give window-stamped derived rows their own window columns instead of borrowing `Task.startDate/endDate`; then retire the columns (Dexie + GRDB, tolerant decode). Context: `Task.timeframe/startDate/endDate` came from Phase 6.Y "Timeboxed Tasks" (PR #64/#93) as a library-hygiene expiry flag; the user-editable "Time window" section was removed from both Task Detail edit sheets in PR #506 (it could un-stamp or move a derived row's window), so today the columns are written only by wizard creation + member-rules stamping and read by `isTaskExpired` + the derived-row kernel. Hand-set windows already stored stay (filter-only).
- **Ended-but-unsealed boards — DECIDED 2026-09-24 (option (c), both), PR #TBD — `fix/root-window-end-bound`:** root squares used to keep absorbing later logs on an ended-but-unsealed board (start-bound-only windows). The owner chose both fixes. (1) WC Decision 1 amended: every board's root squares evaluate events inside its own `[startDate, endDate]` (inclusive, parsed instant; sealing narrows to `min(endDate, sealedAt)`), and a log made from an ended board's own surface is stamped `min(now, endDate)` (`lateLogOccurredAt` / `lateLogStampForBoard`) so it counts there and not for the next window — see `docs/WINDOWED_COMPLETION.md` §Overtime attribution. (2) Ended boards are never Board Sources: `isEligibleSourceBoard(board, now)` admits only open, unsealed boards (the #482 30-day lookback is gone), a series binds to its instance open now (`pickOpenSeriesInstance`), and a source with no open board shows "No board for this window yet" — see `docs/BOARD_SOURCES.md` §Boards as sources.
