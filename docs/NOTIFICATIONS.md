...
Web notifications (PWA + service worker + FCM/VAPID + backend scheduler); bingo/greenlog-moment notifications; streak-milestone notifications (the streak domain model itself shipped — PRs #152/#154 — but no notification is wired to it); server push of any kind.
